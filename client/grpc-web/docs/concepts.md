...
Definitions of the format used by `@alexeyproskuryakov/grpc-web` can be generated using the [`ts-protoc-gen`](https://github.com/alexeyproskuryakov/ts-protoc-gen) plugin for [protoc](https://github.com/google/protobuf). See [code generation](code-generation) for instructions.
...
`@alexeyproskuryakov/grpc-web` uses the [`js-browser-headers`](https://github.com/alexeyproskuryakov/js-browser-headers) library to provide a consistent implementation of the Headers class across browsers. The `BrowserHeaders` class from this library is aliased to `grpc.Metadata`.
